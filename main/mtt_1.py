...
class CustomEmbedding(nn.Module):
    ...
    def __init__(self, n_channels: int, n_dims: int, byte_per_token:int):
        """
        Args:
        ...
        self.n_dims = n_dims
        self.byte_per_token = byte_per_token
        self.embedding = nn.Linear(self.byte_per_token, self.n_dims)
    ...
    def forward(self, input_data):
        ...
        input_data = input_data.reshape(-1, 1440//self.byte_per_token, self.byte_per_token)
...
class MTT_dropping(nn.Module):
    def __init__(
        ...
        self.embedding = CustomEmbedding(embed_n, embed_d, byte_per_token=1440//embed_n)
        ...
        if(keep_rate == 1):
            self.pred_linear = nn.Linear(embed_n*embed_d, n_class)
        else:
            self.pred_linear = nn.Linear(ceil(keep_rate*(embed_n-1))*embed_d+embed_d, n_class)
